...
{
  std::string chords_str = THelper::Stream::StreamOperations::getLineRegardlessLineEnding(in);
  int chords = std::atoi(chords_str.c_str());
  for (int i = 0; i < chords; ++i)
  {
    std::string chord_line = THelper::Stream::StreamOperations::getLineRegardlessLineEnding(in);
    Guitar::Tab tab{in};
